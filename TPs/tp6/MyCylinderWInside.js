/**
 * MyCylinderWInsideWInside
 * @constructor
 */
function MyCylinderWInside(scene, slices, stacks) {
...
MyCylinderWInside.prototype = Object.create(CGFobject.prototype);
MyCylinderWInside.prototype.constructor = MyCylinderWInside;

MyCylinderWInside.prototype.initBuffers = function() {
    this.vertices = [];
    this.indices = [];
    this.normals = [];
    this.texCoords = [];
	...
	for(i = 0; i < this.stacks; i++) {
		for(j = 0; j < this.slices; j++) {
			...
			//Cylinder Inside
			this.indices.push(i*this.slices*2 + j*2, (i+1)*this.slices*2 + j*2, i*this.slices*2 + j*2+1);
			this.indices.push(i*this.slices*2 + j*2+1, (i+1)*this.slices*2 + j*2, (i+1)*this.slices*2 + j*2+1);
		}
	}
